...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    ...
    if (config.nav !== false) {
      if (s.href) {
        const a = el('a', 'sw-nav__item'); a.href = s.href; a.textContent = s.label || '';
        nav.appendChild(a);
      } else {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      c.style.setProperty('--sw-copy-drift', reduce ? '0vh' : `${(0.5 - pr) * 4}vh`);
...
function injectCSS() {
  ...
  .sw-copy{position:absolute;left:clamp(18px,5vw,64px);top:50%;transform:translateY(calc(-50% + var(--sw-copy-drift,0vh)));width:min(42vw,460px);opacity:0;will-change:opacity,transform;}
  ...
  @media (max-width:860px){
    ...
    .sw-copy{left:clamp(18px,5vw,64px);right:clamp(18px,5vw,64px);top:auto;bottom:clamp(64px,14vh,120px);transform:translateY(var(--sw-copy-drift,0vh));width:auto;max-width:560px;}
